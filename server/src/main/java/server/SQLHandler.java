...
public class SQLHandler {
    ...
    private static void prepareAllStatements () throws SQLException {
        psGetNickname = connection.prepareStatement("SELECT nickname FROM users WERE login = ? AND password = ?;");
...
//        }
    ...
    public static boolean changeNick(String oldNickname, String newNickname) {
        try {
            psRegistration.setString(1, newNickname);
            psRegistration.setString(2, oldNickname);
